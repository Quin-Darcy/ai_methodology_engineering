...
- **Carries an expected-behavioral-delta annotation** (`expected_delta` ∈ {`high`, `medium`, `low`, `platitude`}) plus a one-line `delta_rationale`. See 5.4 for the rubric and the design rationale.
...
For each fork, encode a conditional directive of the form: "If [condition A], do X. If [condition B], do Y." Do not resolve the fork by adopting one side. The activating condition is the directive's principal mechanism — without it the conditional carries no usable signal.
...
A canonical directive file: one entry per cluster (or fork), with sources cited, triggers explicit, and an expected-delta annotation. Path: `docs/_manifest/<component>-canonical.yaml`. Per-entry schema is spelled out in `docs/_manifest/canonicalize_prompt_template.md` and includes `directive`, `trigger`, `qualification`, `is_fork`, `expected_delta`, `delta_rationale`, `sources`, `member_ids`.

### 5.4 Expected-delta annotation

Each canonical receives an `expected_delta` rating and a one-line `delta_rationale`. The rating estimates how much an LLM agent's behavior would shift if this directive were included versus omitted, holding everything else constant. Four levels:
...
- **`medium`** — default behavior is partially compliant; the directive sharpens or makes explicit a tendency the agent has but applies inconsistently. Example: "cite primary sources for every nontrivial claim" — Claude cites often but not reliably for every nontrivial claim.
- **`low`** — default behavior is broadly aligned; the directive's value is protective (against drift in long contexts, against sycophancy under pressure, against rare adversarial framings). Example: "give the interlocutor the benefit of the doubt in initial interpretation." Most of the time Claude already does this; the directive defends the tail.
- **`platitude`** — fails *all three* of: (a) surface-verifiable trigger condition, (b) specifiable operation, (c) falsifiable outcome. Example: "be thoughtful," "engage with care." Vagueness alone does not make a directive a platitude; only the simultaneous absence of all three properties does.

**How the annotation gets used.** Stage 7 (compression) uses the rating to make non-arbitrary cuts when over the curse-of-instructions budget: cut `platitude` first, then `low`, then `medium`, and only cut `high` with an explicit reason. The annotation is also part of the audit trail and a hypothesis input to Stage 8 testing — if Stage 8 finds a `high`-rated directive has no measurable effect, that is information about both the directive and the rubric.

**Calibration discipline.** Most directives in a methodological corpus are `medium` or `low`. Reviewers should resist marking everything `high` because the source was authoritative, and resist marking everything `low` because Claude is competent at most things. An inflated rating is more harmful than an honest one, because it leaves Stage 7 with nothing to act on.

### 5.5 Design rationale: what is and is not part of this pipeline

The following design questions were considered and resolved before Stage 5 was scaled. Future agents working in this directory should treat the decisions below as live — i.e., revisit only if a concrete signal emerges that justifies reopening.

**Considered and rejected: a dedicated Stage 4.5 "platitude filter" between clustering and canonicalization.** The proc corpus is overwhelmingly methodological detail (PICO framework selection, search-filter conventions, screening reliability, charting categories) rather than vague exhortation. Spot-checking proc-clusters.yaml suggests fewer than ~15 of 188 canonicals would fail the strict three-test criterion. Formalizing a new pipeline stage (template + builder + LLM pass + audit trail) to remove ~10 directives costs more than the manual review it replaces. The platitude judgment is therefore absorbed into the Stage 5 `expected_delta` annotation (any canonical rated `platitude` is a candidate for drop at Stage 7 review). A standalone Stage 4.5 would be revisited only if the platitude class turned out larger than ~25 canonicals in a later component, where the manual-review cost begins to dominate.

**Considered and deferred: a Stage 9 feedback loop using Stage 8 ablation measurements to re-run Stages 6–7.** The shape is sound — close the loop between Stage 8 measurement and the deliverable — but for the feedback to do real work, Stage 8 must produce reliable per-directive or per-cluster delta measurements (an ablation design, an N large enough for signal, a judging rubric with low variance). The current Stage 8 sketch is adequate for "does the deliverable beat the lightweight baseline" but inadequate for "which directives are pulling weight." Designing Stage 8 well is comparable in size to Stages 5–7 combined. Designing it half-thoroughly produces ablation numbers dominated by noise, and Stage 9 cuts driven by those numbers would be judgment-in-a-lab-coat rather than empirical correction.

Deferring Stage 9 also keeps the v1 deliverable from being scoped to a problem (too many directives, need empirical cuts) that has not yet been observed. The likelier issues in proc.draft.md v1 are coverage gaps, wrong staging structure for the question types actually asked, and over/under-specific triggers — none of which Stage 9 addresses. Reopen the Stage 9 decision after v1 of proc.draft.md is in hand and tested against representative questions; the kind of feedback loop that helps will be much clearer then.

**Kept: the Stage 5 `expected_delta` annotation itself.** Cheap (two fields per canonical), useful even without Stage 8 feedback (forces explicit impact judgments during canonicalization, improves Stage 7 audit), and forward-compatible with a future Stage 9 if that work is later justified.
...
5. **Constraint-count budget per stage.** Within each activation-stage block, count the atomic directives. The "curse of instructions" predicts joint compliance ≈ p^n where p is the per-directive compliance rate and n is the number of distinct atomic directives in play simultaneously. As a working budget, aim for no more than ~10 atomic directives active in any single response (always-on directives plus the activated stage's directives). If a stage block exceeds this, sub-group directives by sub-trigger, merge co-triggering directives, or cut by `expected_delta` (Stage 5.4) — cut `platitude` first, then `low`, then `medium`, and only `high` with an explicit reason recorded in the audit. The 10-directive figure is a working heuristic, not an empirically optimized threshold.
